...
def example_train_capture():
    ...
    # shots=None
    vidmaker = VideoMaker('test', dims=(960, 544), capture_rate=1, shots=shots, max_gpu=False)
    # vidmaker = None
    ...
    dataset = MandelbrotDataSet(20000, max_depth=200, gpu=False)
    # dataset = MandelbrotDataSet(loadfile='500k_inv')


    train(model, dataset, 1, batch_size=2000, 
        use_scheduler=True, oversample=0.1,
        snapshots_every=200, vm=vidmaker)
